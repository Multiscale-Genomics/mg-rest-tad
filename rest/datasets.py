...
class datasets:
    ...
    def __init__(self):
        ...
        self.binSizes = [1000, 5000, 10000, 25000, 50000, 100000, 250000, 500000, 1000000]
        ...
        for taxon_id in self.datasets["taxon_id"]:
            for accession_id in self.datasets["taxon_id"][taxon_id]["accession"]:
                ...
                for i in xrange(len(self.datasets["taxon_id"][taxon_id]["accession"][accession_id]["chromosomes"])):
                    ...
                    binS = [int(np.ceil(c[1]/float(y))) for y in self.binSizes]
                    binC = dict(zip(self.binSizes, [[binS[j], binCount[j], binS[j]+binCount[j]] for j in range(len(binCount))]))
                ...
                totalBinCount = dict(zip(self.binSizes, [[binS[i], binCount[i]]for i in range(len(binCount))]))
    ...
    def getResolutions(self):
        """
        Return a list of the potential resolutions
        """
    ...
    def getBinSizes(self):
        """
        Return the list of the resolutions. This is what the TADs are calcualted
        for. If the data has not been calculated then it can still be requested,
